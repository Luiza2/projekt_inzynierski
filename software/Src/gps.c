...
void gps_recv_char(struct gps_state * state, uint8_t recv_char) {
	if (state->writer_position == 0 && recv_char == '$') {
		...
		if (recv_char == '\r' || recv_char == '\n') {
			...
			//kolejne znaki zapisywane na kolejnych pozycjach bufora
...
//odczytuje pola po znaku przecinka lub konca ciagu
...
//odczytuje typ ramki danych i wywoluje odpowiednia funkcje ja przetwarzajaca
...
void gps_process_gprmc(struct gps_state * state)
{
	...
	if(strlen(state->field_buffer) > 0) {
		...
		state->time_hour = (tmp / 10000) % 100; //utc
	}
